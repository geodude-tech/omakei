...
**Do not write to the database yourself** — not a `categoryId`, not a rule.
The app re-derives every category on every load and every folder sync
...
editor can save straight over it. Change categories by changing rules, through
the tool.
